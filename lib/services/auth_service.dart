...
class AuthService extends ChangeNotifier {
  ...
  Future<AuthResult> signUp({
    ...
    try {
      ...
      if (user == null) {
        return AuthResult.failure("Sign up failed: Unknown error");
      }
  ...
  Future<AuthResult> deleteAccount({String? currentPassword}) async {
    try {
      ...
      // 4. Clean up social relationships
      try {
        // Users who follow this deleted user
        final followerDocs = await _db
            .collection('users')
            .where('following', arrayContains: uid)
            .get();
        for (final doc in followerDocs.docs) {
          final f = List<String>.from(doc['following'] ?? []);
          f.remove(uid);
          await _db.collection('users').doc(doc.id).update({'following': f});
        }
        // Users whom this deleted user follows
        final followingDocs = await _db
            .collection('users')
            .where('followers', arrayContains: uid)
            .get();
        for (final doc in followingDocs.docs) {
          final f = List<String>.from(doc['followers'] ?? []);
          f.remove(uid);
          await _db.collection('users').doc(doc.id).update({'followers': f});
        }
      } catch (e) {
        debugPrint(
          "Error cleaning up social relationships during account deletion: $e",
        );
      }
